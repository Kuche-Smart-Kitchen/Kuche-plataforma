...
const electroCategoriesRoutes = {
  base: ["/api/electrodomesticos/categorias", "/api/electro-categorias", "/api/catalogos/electro-categorias"],
};
...
const getLastPathErrorMessage = (errors: unknown[]) => {
  ...
  const hasNetworkError = errors.some((currentError: any) => Boolean(currentError?.request) && !currentError?.response);
  ...
  if (hasNetworkError) {
    const backendUrl =
      ...
      (process.env.NODE_ENV === "production"
        ...
        : "http://localhost:3001");
    return `No se pudo conectar con el backend (${backendUrl}). Verifica que el servidor API esté iniciado.`;
  }

  return (
    lastError?.response?.data?.message ||
    lastError?.message ||
    "No se pudo completar la operación de equipamiento"
  );
};
